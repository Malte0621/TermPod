...
pod_file_pod6_t* pod_file_pod6_create(pod_string_t filename)
{
	...
	{
		pod_file->data = calloc(1, POD_HEADER_POD6_SIZE);
		pod_file->header = (pod_header_pod6_t*)pod_file->data;
		memcpy(pod_file->header, "POD6", POD_IDENT_SIZE);
		pod_file->header->file_count = 0;
		pod_file->header->version = 0;
		pod_file->header->index_offset = POD_HEADER_POD6_SIZE;
		pod_file->header->size_index = 0x110;
		
		pod_file->entries = calloc(1, 1); // (pod_entry_pod6_t*)(pod_file->data + pod_file->header->index_offset);
		pod_file->entry_data = (pod_byte_t*)(pod_file->data + pod_file->header->index_offset + pod_file->header->file_count * POD_DIR_ENTRY_POD6_SIZE);
		pod_file->entry_data_size = 0;
		pod_file->path_data = calloc(1, 1);
		...
		pod_file->checksum = 0;
		pod_file->size = POD_HEADER_POD6_SIZE;
		return pod_file;
	}
...
bool pod_file_pod6_add_entry(pod_file_pod6_t* pod_file, pod_entry_pod6_t* entry, pod_string_t filename, pod_byte_t* data)
{
	...
	size_t new_entry_size = POD_DIR_ENTRY_POD6_SIZE;
	...
	//pod_file->checksum = pod_crc_pod6(pod_file);
	
	return true;

}

bool pod_file_pod6_del_entry(pod_file_pod6_t* pod_file, pod_number_t entry_index)
{
	if (pod_file == NULL)
	{
		fprintf(stderr, "ERROR: pod_file_pod6_del_entry(pod_file == NULL)\n");
		return false;
	}

	if (entry_index >= pod_file->header->file_count)
	{
		fprintf(stderr, "ERROR: pod_file_pod6_del_entry(entry_index >= pod_file->header->file_count)\n");
		return false;
	}

	pod_entry_pod6_t* entry = &pod_file->entries[entry_index];
	...
	{
		memmove(pod_file->entries + entry_index, pod_file->entries + entry_index + 1, (pod_file->header->file_count - entry_index - 1) * POD_DIR_ENTRY_POD6_SIZE);
	}

	pod_file->header->file_count--;
	pod_file->entries = realloc(pod_file->entries, pod_file->header->file_count * POD_DIR_ENTRY_POD6_SIZE);
...
pod_entry_pod6_t* pod_file_pod6_get_entry(pod_file_pod6_t* pod_file, pod_number_t entry_index)
{
	if (pod_file == NULL)
	{
		fprintf(stderr, "ERROR: pod_file_pod6_get_entry(pod_file == NULL)\n");
		return NULL;
	}

	if (entry_index >= pod_file->header->file_count)
	{
		fprintf(stderr, "ERROR: pod_file_pod6_get_entry(entry_index >= pod_file->header->file_count)\n");
		return NULL;
	}
...
bool pod_file_pod6_extract_entry(pod_file_pod6_t* pod_file, pod_number_t entry_index, pod_string_t dst)
{
	if (pod_file == NULL)
	{
		fprintf(stderr, "ERROR: pod_file_pod6_extract_entry(pod_file == NULL)\n");
		return false;
	}

	if (entry_index >= pod_file->header->file_count)
	{
		fprintf(stderr, "ERROR: pod_file_pod6_extract_entry(entry_index >= pod_file->header->file_count)\n");
		return false;
	}
	...
	if (file == NULL)
	{
		fprintf(stderr, "ERROR: pod_file_pod6_extract_entry(fopen(%s, \"wb\") failed: %s\n", dst, strerror(errno));
		return false;
	}
	...
	if (data == NULL) {
		...
		{
			fprintf(stderr, "ERROR: pod_file_pod6_extract_entry() fwrite failed!\n");
			fclose(file);
			return false;
		}
	}
...
bool pod_file_pod6_print(pod_file_pod6_t* pod_file, pod_char_t* pattern)
{
	if(pod_file == NULL)
	{
...
/* Extract POD6 file pod_file to directory dst                       */
/* @returns true on success otherwise false and leaves errno         */
bool pod_file_pod6_extract(pod_file_pod6_t* pod_file, pod_string_t pattern, pod_string_t dst)
{
	pod_bool_t absolute = false;
	if (pod_file == NULL)
	{
		fprintf(stderr, "ERROR: pod_file_pod6_extract(pod_file == NULL)\n");
		return false;
	}
	...
	for (int i = 0; i < pod_file->header->file_count; i++)
	{
		...
		if (!filename)
		{
			fprintf(stderr, "ERROR: pod_file_pod6_extract(filename == NULL)\n");
			return false;
		}
		...
		if (data == NULL) {
			if (fwrite(pod_file->data + entry->offset, entry->size, 1, file) != 1)
			{
				fprintf(stderr, "ERROR: pod_file_pod6_extract() fwrite failed!\n");
				fclose(file);
				return false;
			}
		}
...
bool pod_file_pod6_write(pod_file_pod6_t* pod_file, pod_string_t filename)
{
	...
	size_t new_total_size = pod_file->size + pod_file->path_data_size + pod_file->header->file_count * POD_DIR_ENTRY_POD6_SIZE;
	void* new_data = realloc(pod_file->data, new_total_size);
	if (!new_data) {
	...
	memcpy(pod_file->data + pod_file->size, pod_file->entries, pod_file->header->file_count * POD_DIR_ENTRY_POD6_SIZE);
	pod_file->size += pod_file->header->file_count * POD_DIR_ENTRY_POD6_SIZE;
	...
	{
		fprintf(stderr, "ERROR: writing POD6 data!\n");
		fclose(file);
		return false;
	}
