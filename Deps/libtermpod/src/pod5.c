...
pod_checksum_t pod_crc_pod5(pod_file_pod5_t* file)
{
...
pod_checksum_t pod_crc_pod5_entry(pod_file_pod5_t* file, pod_number_t entry_index)
{
...
pod_checksum_t pod_crc_pod5_audit_entry(pod_file_pod5_t* file, pod_number_t audit_entry_index)
{
	if(file == NULL || file->entry_data == NULL)
	{
		fprintf(stderr, "ERROR: pod_crc_pod5() file == NULL!");
...
pod_checksum_t   pod_file_pod5_chksum(pod_file_pod5_t* podfile)
{
	return pod_crc_pod5(podfile);
}
...
pod_file_pod5_t* pod_file_pod5_create(pod_string_t filename)
{
	...
	{
		pod_file->data = calloc(1, POD_HEADER_POD5_SIZE);
		pod_file->header = (pod_header_pod5_t*)pod_file->data;
		memcpy(pod_file->header, "POD5", POD_IDENT_SIZE);
		...
		pod_file->header->index_offset = POD_HEADER_POD5_SIZE;
		pod_file->header->unknown10c = 0x10c;
		pod_file->header->size_index = 0x110;
		pod_file->header->flag0 = 0;
		pod_file->header->flag1 = 0;
		pod_file->header->pad11c = 0xFFFFFFFF;
		pod_file->header->next_archive[0] = '\0';
		pod_file->header->next_archive[POD_HEADER_NEXT_ARCHIVE_SIZE - 1] = '\0';

		pod_file->entries = calloc(1, sizeof(pod_entry_pod5_t));
		pod_file->entry_data = (pod_byte_t*)(pod_file->data + pod_file->header->index_offset + pod_file->header->file_count * POD_DIR_ENTRY_POD5_SIZE);
		pod_file->entry_data_size = 0;
		pod_file->path_data = calloc(1, sizeof(pod_char_t));
		pod_file->audit_trail = calloc(1, sizeof(pod_number_t));
		pod_file->audit_data_size = 0;
		pod_file->gap_sizes = calloc(1, sizeof(pod_number_t));
		pod_file->checksum = 0;
		pod_file->size = POD_HEADER_POD5_SIZE;
		return pod_file;
	}
...
bool pod_file_pod5_add_entry(pod_file_pod5_t* pod_file, pod_entry_pod5_t* entry, pod_string_t filename, pod_byte_t* data)
{
	...
	size_t new_entry_size = POD_DIR_ENTRY_POD5_SIZE;
	...
	//pod_file->checksum = pod_crc_pod5(pod_file);

	return true;
}

bool pod_file_pod5_del_entry(pod_file_pod5_t* pod_file, pod_number_t entry_index)
{
	if (pod_file == NULL)
	{
		fprintf(stderr, "ERROR: pod_file_pod5_del_entry(pod_file == NULL)\n");
		return false;
	}

	if (entry_index >= pod_file->header->file_count)
	{
		fprintf(stderr, "ERROR: pod_file_pod5_del_entry(entry_index >= pod_file->header->file_count)\n");
		return false;
	}

	pod_entry_pod5_t* entry = &pod_file->entries[entry_index];
	...
	{
		memmove(pod_file->entries + entry_index, pod_file->entries + entry_index + 1, (pod_file->header->file_count - entry_index - 1) * POD_DIR_ENTRY_POD5_SIZE);
	}

	pod_file->header->file_count--;
	pod_file->entries = realloc(pod_file->entries, pod_file->header->file_count * POD_DIR_ENTRY_POD5_SIZE);
...
pod_entry_pod5_t* pod_file_pod5_get_entry(pod_file_pod5_t* pod_file, pod_number_t entry_index)
{
	if (pod_file == NULL)
	{
		fprintf(stderr, "ERROR: pod_file_pod5_get_entry(pod_file == NULL)\n");
		return NULL;
	}

	if (entry_index >= pod_file->header->file_count)
	{
		fprintf(stderr, "ERROR: pod_file_pod5_get_entry(entry_index >= pod_file->header->file_count)\n");
		return NULL;
	}
...
bool pod_file_pod5_extract_entry(pod_file_pod5_t* pod_file, pod_number_t entry_index, pod_string_t dst)
{
	if (pod_file == NULL)
	{
		fprintf(stderr, "ERROR: pod_file_pod5_extract_entry(pod_file == NULL)\n");
		return false;
	}

	if (entry_index >= pod_file->header->file_count)
	{
		fprintf(stderr, "ERROR: pod_file_pod5_extract_entry(entry_index >= pod_file->header->file_count)\n");
		return false;
	}
	...
	if (file == NULL)
	{
		fprintf(stderr, "ERROR: pod_file_pod5_extract_entry(fopen(%s, \"wb\") failed: %s\n", dst, strerror(errno));
		return false;
	}
	...
	if (data == NULL) {
		...
		{
			fprintf(stderr, "ERROR: pod_file_pod5_extract_entry() fwrite failed!\n");
			fclose(file);
			return false;
		}
	}
...
pod_file_pod5_t* pod_file_pod5_delete(pod_file_pod5_t* podfile)
{
	if(podfile)
	{
...
bool pod_file_pod5_print(pod_file_pod5_t* pod_file, pod_char_t* pattern)
{
	if(pod_file == NULL)
	{
	...
	if(pod_file->audit_trail != NULL || pod_file->audit_data_size > 0)
	{
		...
		for(int i = 0; i < pod_file->header->audit_file_count; i++)
		{
			if(!pod_audit_entry_pod5_print(&pod_file->audit_trail[i]))
			{
				...
				return pod_file_pod5_delete(pod_file);
			}
		}
	}
...
/* Extract POD5 file pod_file to directory dst                       */
/* @returns true on success otherwise false and leaves errno         */
bool pod_file_pod5_extract(pod_file_pod5_t* pod_file, pod_string_t pattern, pod_string_t dst)
{
	pod_bool_t absolute = false;
	if (pod_file == NULL)
	{
		fprintf(stderr, "ERROR: pod_file_pod5_extract(pod_file == NULL)\n");
		return false;
	}
	...
	for (int i = 0; i < pod_file->header->file_count; i++)
	{
		...
		if (!filename)
		{
			fprintf(stderr, "ERROR: pod_file_pod5_extract(filename == NULL)\n");
			return false;
		}
		...
		if (data == NULL) {
			if (fwrite(pod_file->data + entry->offset, entry->size, 1, file) != 1)
			{
				fprintf(stderr, "ERROR: pod_file_pod5_extract() fwrite failed!\n");
				fclose(file);
				return false;
			}
		}
...
bool pod_file_pod5_write(pod_file_pod5_t* pod_file, pod_string_t filename)
{
	...
	size_t new_total_size = pod_file->size + pod_file->path_data_size + pod_file->header->file_count * POD_DIR_ENTRY_POD5_SIZE;
	void* new_data = realloc(pod_file->data, new_total_size);
	if (!new_data) {
	...
	memcpy(pod_file->data + pod_file->size, pod_file->entries, pod_file->header->file_count * POD_DIR_ENTRY_POD5_SIZE);
	pod_file->size += pod_file->header->file_count * POD_DIR_ENTRY_POD5_SIZE;
	...
	{
		fprintf(stderr, "ERROR: writing POD5 data!\n");
		fclose(file);
		return false;
	}
