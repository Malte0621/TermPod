...
pod_ssize_t pod_zip_compress(pod_byte_t** dst, pod_byte_t* src, pod_size_t decompressed)
{
	...
	/* create source from buffer */
	zip_source_t* zip_src = zip_source_buffer_create(src, decompressed, 1, &zip_error);
	...
	else
	{
		...
		if ((*dst = calloc(1, compressed)) == NULL)
		{
			fprintf(stderr, "calloc failed: %s\n", strerror(errno));
			zip_source_close(zip_src);
			return -1;
		}
		if ((zip_uint64_t)zip_source_read(zip_src, *dst, compressed) < compressed)
		{
